...
export class AcademicService {
  ...
  async findAllActividades(
    ...
    const queryBuilder = this.actividadRepository
      ...
      .leftJoinAndSelect('actividad.grupo', 'grupo')
      .leftJoinAndSelect('actividad.periodo_academico', 'periodo_academico')
      .leftJoinAndSelect('actividad.estado_actividad', 'estado_actividad');

    if (filters?.asignatura_id) {
      queryBuilder.andWhere('actividad.asignatura_id = :asignatura_id', {
        asignatura_id: filters.asignatura_id,
      });
    }

    if (filters?.grupo_id) {
      queryBuilder.andWhere('actividad.grupo_id = :grupo_id', {
        grupo_id: filters.grupo_id,
      });
    }

    if (filters?.periodo_id) {
      queryBuilder.andWhere('actividad.periodo_academico_id = :periodo_id', {
  ...
  async updateActividad(
    id: number,
    updateData: Partial<CreateActividadDto>,
  ): Promise<Actividad> {
  ...
  async findAllAsignaturas(
    ...
    const queryBuilder = this.asignaturaRepository
      ...
      .leftJoinAndSelect('asignatura.nivel_academico', 'nivel_academico');
  ...
  async findOneAsignatura(id: number): Promise<Asignatura> {
    const asignatura = await this.asignaturaRepository.findOne({
      where: { id },
      relations: ['nivel_academico', 'actividades'],
    });
  ...
  async updateAsignatura(
    id: number,
    updateData: Partial<CreateAsignaturaDto>,
  ): Promise<Asignatura> {
